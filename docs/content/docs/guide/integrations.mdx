...
description: Point any OpenAI-compatible coding tool at your llmrun deployment.
---
...
<Callout type="info" title="Forward must be active">
...
<Callout type="info" title="Agent mode needs tool_call_parser">
...
unless the model was launched with those flags. Set `tool_call_parser` (e.g. `hermes` for Qwen models) on the model entry in `llmrun.yaml` and re-provision — see [Tool / function calling](./model-catalog.mdx#tool--function-calling).
