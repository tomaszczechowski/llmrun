...
| `cpu_fallback`      | —       | CPU fallback block (`instance_type`, `engine: ollama\|vllm`, `quantization`, `max_params`, `context_length`, `vllm` tuning) — used when GPU quota is unavailable; see [Model Catalog](./model-catalog.mdx#cpu-fallback) |
